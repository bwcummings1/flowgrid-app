...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!!data?.ok) {
      ...
      const userId = data?.auth_user?.id
      const userToken = data?.auth_user?.access_token
      ...
      return NextResponse.redirect(
        `https://localhost:3000/connections?app_id=${appId}&auth_user_id=${userId}&auth_user_token=${userToken}&slack_access_token=${accessToken}&bot_user_id=${botUserId}&team_id=${teamId}&team_name=${teamName}`
      )
    }
  } catch (error) {
    console.error(error)
